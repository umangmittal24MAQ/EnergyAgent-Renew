...
def export_unified_xlsx(df: pd.DataFrame) -> bytes:
    """
    Export unified dataframe to Excel format (alias for export_to_excel).
    """
    return export_to_excel(df, include_metadata=True)


def export_ecs_style_xlsx(df: pd.DataFrame) -> bytes:
    """
    Export dataframe in ECS style format to Excel.
    This matches the Electrical Optimization sheet format.
    """
    return export_to_excel(df, include_metadata=False)
