...
# Cache DB path
CACHE_DB_PATH = Path(__file__).parent.parent.parent / "energy-dashboard" / "output" / ".cache"
...
class CacheService:
    ...
    def _persist_to_db(self, key: str, value: Any, ttl_seconds: int):
        ...
        try:
            ...
            logger.warning(f"Failed to persist {key} to database: {e}")
    ...
    def _remove_from_db(self, key: str):
        ...
        try:
            ...
            logger.warning(f"Failed to remove {key} from database: {e}")
